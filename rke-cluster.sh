...
function _CILIUM_NOT_CANAL {
	if echo $RKE2_VERSION |grep "v1.20.6" && [ -f $RKECLUSTERDIR/manifests/rke2-cilium.yaml ]; then
		...
		echo "Cilium Yaml exists and cluster version is v1.20.7-v1.20.15 or v1.21.2-v1.24.4"
		...
		echo "Cilium Yaml does not exist or cluster version is not v1.20.7-v1.20.15 or v1.21.2-v1.24.4"
		sudo rm $RKECLUSTERDIR/manifests/rke2-cilium*.yaml*
	fi
}
