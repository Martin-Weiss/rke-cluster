...
function _FIX_1_20_11 {
        if echo $RKE2_VERSION |grep "v1.20.11" || echo $RKE2_VERSION |grep "v1.20.12" || echo $RKE2_VERSION |grep "v1.20.13"; then
...
function _CILIUM_NOT_CANAL {
	if echo $RKE2_VERSION |grep "v1.20.6" && [ -f $RKECLUSTERDIR/manifests/rke2-cilium.yaml ]; then
		...
		echo "Cilium Yaml does not exist or cluster version is not v1.20.7-v1.20.13 or v1.21.2-v1.21.7"
		sudo rm $RKECLUSTERDIR/manifests/rke2-cilium*.yaml*
	fi
}
